...
describe("CourseCard component", () => {
    ...
    it('CourseCard should display duration, in right format', () => {
        ...
        expect(screen.getByText(formatDuration(propsCourseCard.duration) + ' hours')).toBeInTheDocument();
        const hours = Math.floor(propsCourseCard.duration / 60);
        const minutes = propsCourseCard.duration % 60;
        const expectedFormat = `${hours.toString().padStart(2, '0')}:${minutes.toString().padStart(2, '0')} hours`;
    ...
    it("CourseCard should display authors list", () => {
        ...
        propsCourseCard.authors.forEach((e, index) => {
            ...
            return authorListString += selectAuthor(e, initialState.authors) + format;
        })
    ...
    it(" CourseCard should display created date in the correct format", () => {
        ...
        expect(screen.getByText(formatDate(propsCourseCard.creationDate))).toBeInTheDocument();
    })
});
